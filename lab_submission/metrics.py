...
def scorer(solution, prediction, metric):
    score = None
    if metric == 'false_negative_rate':
        score = false_negative_rate(solution, prediction)
    elif metric == 'inv_auroc_score':
        score = inv_auroc_score(solution, prediction)
    elif metric == 'inv_f1_score':
        score = inv_f1_score(solution, prediction)
    elif metric == 'inv_map_score':
        score = inv_map_score(solution, prediction)
    elif metric == 'l2_relative_error':
        score = l2_relative_error(solution, prediction)
    elif metric == 'nll_score':
        score = nll_score(solution, prediction)
    elif metric == 'zero_one_error':
        score = zero_one_error(solution, prediction)
    else:
        NotImplementedError(f'{metric} metric not implemented')
    return score
